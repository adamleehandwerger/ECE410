...
$\int_0^{x_j} (\sum_i \alpha_i)(\sum_j \alpha_j) \,dt=(\sum_k \alpha_k)^2 x_j \geq 0$ for $x_j \in \mathbb{R}^{+}$.\\
    \vspace{.4cm}
...
$Var(\sum_i \alpha_i \mathbb{I}_{A_i}(\omega)) \geq 0$.\\
\vspace{.4cm}
Credit: https://en.wikipedia.org/wiki/Covariance.\\
*Copied formula incorrectly first time.\\
\vspace{.4cm}
...
$=\alpha_1^2 \min\{x_p\}_1-2 \alpha_1^2 \min(\{x_p\}_1 \cap \{x_p\}_2)+\alpha_1^2 \min\{x_p\}_2=$\\
$=\alpha_1^2\left(\min\{x_p\}_1+\min\{x_p\}_2-2 \min(\{x_p\}_1 \cap \{x_p\}_2)\right) \leq 0$\\
\vspace{.4cm}
*had indices labeled i,j instead of 1,2 first time.\\
\vspace{.4cm}
...
\vspace{.4cm}
$\sum_{ij}^{n} \alpha_i \alpha_j k(x_i,x_j)=\sum_{ij}^{n} \alpha_i \alpha_j E \left(e^{(x_i-x_j)Z}\right) = E\left(\sum_{ij}^{n}\alpha_i \alpha_j(e^{iZx_i} e^{-iZx_j})\right)=$\\
$E\left(\sum_{i}^{n} \alpha_i e^{iZx_i}\sum_{j}^{n}\alpha_j e^{-iZx_j}\right)=E\left(\sum_k \alpha_k e^{iZx_k}\right)^2 \geq 0$\\
\vspace{.4cm}
...
$E\left(\sum_{i}^{n}\alpha_i e^{iZx_i/\sigma}\sum_{j}^{n}\alpha_j e^{-iZx_j/\sigma}\right)=E\left(\sum_k \alpha_k e^{iZx_k/\sigma}\right)^2 \geq 0$\\
\vspace{.4cm}
*added missing $\alpha_i$'s from the first version
