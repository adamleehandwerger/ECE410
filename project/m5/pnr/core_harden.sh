...
# --- Pull latest caravel changes (picks up RAM_LATENCY=3 config fix) ---
...
# --- Run OpenLane 2 ---
echo "--- Running openlane (RAM_LATENCY=3) ---"
openlane \
    --pdk sky130A \
    --pdk-root $PDK_ROOT \
    --run-tag core_harden \
...
echo "=== Output files ==="
find $OUT_BASE -name "*.gds" -o -name "*.lef" 2>/dev/null | grep -i final | head -10
...
echo "=== core_harden complete — submit wrapper_harden next ==="
