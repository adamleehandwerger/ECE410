...
For \(F \in [0,\; 1)\), a degree-3 Horner polynomial gives
10-bit accuracy:
\[
    e^{-F} \;\approx\; c_0 + F\bigl(c_1 + F(c_2 + F \cdot c_3)\bigr),
\]
where the coefficients \(c_0, \ldots, c_3\) are stored in \(\mathbb{Q}_{6.10}\).
Evaluation proceeds right-to-left (Horner's method), requiring
3 multiplications and 3 additions, totalling 6 fixed-point operations
and 18 clock cycles.
