...
The accumulator is 32-bit. The theoretical worst-case value for \(N=256\)
and maximum feature magnitude \(|x_i - sv_i| \leq 63.999\) is
\[
    D_{\max} = 256 \times (63.999)^2 \approx 1{,}048{,}544 \;<\; 2^{20},
\]
so 20 significant bits are sufficient. If the accumulator reaches
\(\texttt{0xFFFFF}\) (saturated), the sticky flag
\texttt{ERR\_DIST\_OVERFLOW} is asserted.
...
and evaluates them via Horner's method:
\[
    e^{-F} \;\approx\; \sum_{n=0}^{d} \frac{(-F)^n}{n!}
    \;=\; c_0 + F\bigl(c_1 + F(c_2 + \cdots + F \cdot c_d)\bigr),
\]
where the coefficients are the Taylor series values \(c_n = 1/n!\) with
alternating signs applied during evaluation. Evaluation proceeds
right-to-left (Horner's method), requiring \(d\) multiplications and
\(d\) additions.
