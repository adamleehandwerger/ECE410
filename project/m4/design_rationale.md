...
The drain cycles were discovered during m2 testbench development. The distance-zero unit
...
## 5. Why m2 Did Not Provide a Correct Implementation

Milestone 2 targeted RTL verification and synthesis only (no place-and-route). It reported
98.67% accuracy and 19/19 tests passing. However, the m2 architecture had three problems
that made it unsuitable for the Caravel silicon target.

**Problem 1: Decision function computed by the MCU, not the ASIC.**  
In m2, the compute core output raw kernel values K(x, svᵢ) per SV to an off-chip workspace
...
step — the most numerically expensive part (Σ αᵢ K) was still done in software. The m3/m4
...
m2 specified an 8192-word (16 KB) synchronous FIFO for QSPI input buffering. Implementing
...
cell count for the entire core. The m3 architecture replaced the FIFO with a 256-word
...
m2 used a QSPI interface to stream feature data beat-by-beat from the MCU to the FIFO.
...
in the m2 model. See Section 6 for the full interface evolution.
