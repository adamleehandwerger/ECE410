...
produces a raw 32-bit product in \(\mathbb{Q}_{12.20}\) format, because both the
integer and fractional bit-widths double:
\[
    6 + 6 = 12 \text{ integer bits}, \qquad 10 + 10 = 20 \text{ fractional bits}.
\]
To restore \(\mathbb{Q}_{6.10}\), the 32-bit product is arithmetic-right-shifted by
10 bits (dividing by \(2^{10}\)), yielding a \(\mathbb{Q}_{12.10}\) value:
\[
    \widehat{a \cdot b} = \frac{\hat{a} \cdot \hat{b}}{2^{10}}.
\]
The upper bits are then truncated to the 16-bit \(\mathbb{Q}_{6.10}\) range
\([-32768,\; 32767]\). This truncation discards the redundant sign-extension bits
introduced by the wider product; it does \emph{not} lose precision beyond the
10-bit right-shift already applied. The right-shift is arithmetic (sign-preserving)
so that negative products are handled correctly.
...
\texttt{horner}: multiplications & 6 & 500 & 3{,}000 \\
\texttt{horner}: additions & 6 & 500 & 3{,}000 \\
Alpha multiply-accumulate & 1 & 500 & 500 \\
\midrule
\textbf{Total per beat} & & & \textbf{390{,}500} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Kernel FSM cycle count (\(T_{\text{kernel}} = 18\))}

The \texttt{horner} FSM contains the following states, each consuming one
clock cycle:

\begin{center}
\begin{tabular}{lrl}
\toprule
\textbf{States} & \textbf{Cycles} & \textbf{Operation} \\
\midrule
\texttt{SCALE}, \texttt{SCALE2} & 2 & compute \(\hat{p} = \hat{\gamma}\cdot\hat{d}^2 / 2^{10}\); split \(I\) and \(F\) \\
\texttt{HORNER\_14}\,--\,\texttt{HORNER\_0} & 15 & Horner recurrence steps (degree-14 RTL polynomial) \\
\texttt{OUTPUT} & 1 & multiply \(\texttt{lut}[I] \cdot \hat{e}^{-F}\) and register \(\hat{K}\) \\
\midrule
...
The RTL defines 15 \texttt{HORNER} states (\texttt{HORNER\_14} down to
\texttt{HORNER\_0}), corresponding to a degree-14 polynomial evaluation.
Coefficients \(a_{7}\) through \(a_{14}\) truncate to zero in
\(\mathbb{Q}_{6.10}\) (values \(< 2^{-10}\)), so the effective
approximation is degree 6, but the FSM executes all 15 steps regardless.

\subsection*{Total cycle budget}
