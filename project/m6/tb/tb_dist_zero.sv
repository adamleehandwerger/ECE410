// tb_dist_zero.sv — m6 RAM interface
// feature = sv = 0x0400 → dist=0 → kernel_out=1024 (exp(0)=1.0 in Q6.10)
`timescale 1ns/1ps
...
// LAT=1 SRAM: SV=input=0x0400 → dist=0 → kernel=1024
...
initial begin
    ...
    for (int k=0; k<NSV; k++) begin
        if (kernel_values[k]===EXPECTED_KERNEL) $display("  [PASS] kernel[%0d]=%0d", k, kernel_values[k]);
        else $fatal(1,"[FAIL] kernel[%0d]=%0d expected %0d", k, kernel_values[k], EXPECTED_KERNEL);
    end
    $display("tb_dist_zero: PASS"); $finish;
end
initial begin #500_000; $fatal(1,"[FATAL] tb_dist_zero: timeout"); end
endmodule
