"""
tb_wb_unit.py — Wishbone unit tests (m5 v9 interface)
======================================================
Targeted interface tests replacing the legacy QSPI/FIFO tests from m3.
...
Tests
-----
  test_wb_reset_outputs    — STATUS=0 after reset           (m3: test_reset_outputs)
  test_wb_gamma_register   — PARAM_WR programs gamma        (m3: test_param_programming)
  test_wb_num_sv_registers — NUM_SV[0-4] accept writes      (m3: test_sv_counts_set)
  test_wb_alpha_load       — ALPHA_WR loads 25 coefficients (m3: test_sv_counts_unequal_stress)
  test_wb_sram_load        — SRAM pipeline: load + fire + sample_rdy (m3: test_qspi_fifo_load)
  test_wb_ram_latency      — 2-cycle delayed SRAM still classifies   (m3: test_qspi_backpressure)
...
async def test_wb_reset_outputs(dut):
    """STATUS register is zero immediately after reset. (m3: test_reset_outputs)"""
...
async def test_wb_gamma_register(dut):
    """PARAM_WR programs gamma=0.25 correctly in Q6.10. (m3: test_param_programming)"""
    cocotb.start_soon(Clock(dut.wb_clk_i, 25, unit="ns").start())
    await reset_dut(dut)
...
async def test_wb_num_sv_registers(dut):
    """NUM_SV[0-4] accept writes and total is correct. (m3: test_sv_counts_set)"""
    cocotb.start_soon(Clock(dut.wb_clk_i, 25, unit="ns").start())
    await reset_dut(dut)
...
async def test_wb_alpha_load(dut):
    """ALPHA_WR loads 25 alpha coefficients without error. (m3: test_sv_counts_unequal_stress)"""
    cocotb.start_soon(Clock(dut.wb_clk_i, 25, unit="ns").start())
    await reset_dut(dut)
...
async def test_wb_sram_load(dut):
    ...
    Replaces m3 test_qspi_fifo_load — verifies the full load→classify path
...
async def test_wb_ram_latency(dut):
    ...
    Replaces m3 test_qspi_backpressure — verifies the RAM_LATENCY wait-state
