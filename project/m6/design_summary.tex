...
\begin{align}
  T &= F(\Lat+1) + \SV\!\bigl[F(\Lat+1) + 30\bigr] + 500 \notag\\
    &= 256\cdot4 + 500\cdot(256\cdot4+30) + 500 \notag\\
    &= 1024 + 500\cdot1054 + 500 \notag\\
    &= \mathbf{528{,}524\text{ cycles/beat}}. \label{eq:cycles}
\end{align}
At $f_\text{clk}=40\MHz$: $T/f = 13.21\,\mathrm{ms/beat}$, well within the
...
During SVM inference all $\SV=500$ support vector comparisons are active.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrl}
    ...
    \midrule
    SVM core (500 SVs, LAT=3) & $\approx 24.5\mW$ & 1.76\% & $\approx 0.43\mW$ \\
    ...
    \midrule
    \textbf{Total} & --- & --- & $\approx\mathbf{1.03\mW}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{equation}
  t_{\text{battery}} = \frac{740\,\mathrm{mWh}}{1.03\,\mathrm{mW}} \approx 718\,\mathrm{h} \approx 29.9\,\mathrm{days}.
  \label{eq:battery}
\end{equation}
The 14-day target is met with $2.14\times$ margin.  Compared to the m5 sky130A
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Avg power @ 80\,bpm & 0.869\,mW & \textbf{$\approx$0.43\,mW} \\
    Inference time (LAT=3) & 9.87\,ms/beat & \textbf{13.21\,ms/beat} \\
