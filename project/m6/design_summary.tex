...
classifier achieves \textbf{98.67\%} accuracy (296/300) on the held-out test set,
...
SVM is trained independently, allowing per-class support vector budgets
$|\mathcal{S}_k| = 120$ for all $k$ (600 SVs total).
...
\begin{table}[H]
  \centering
  \caption{SV count sweep --- float vs.\ Q6.10 accuracy.}
  \label{tab:quant}
  \small
  \begin{tabular}{rrrrl}
    \toprule
...
The N=120 per-class point is optimal: it eliminates the quantization flip seen at
N=100 while also improving float accuracy.  This motivates the 10-bit
\texttt{alpha\_addr} field (up from 9-bit in m5), giving an address space of
$2^{10}=1024 > 600$.
...
\begin{table}[H]
  \centering
  \caption{Per-class test accuracy at Q6.10 fixed-point, $\SV=600$,
           allocation $[120,120,120,120,120]$.}
  \label{tab:accuracy}
  \small
  \begin{tabular}{lrrl}
    \toprule
    ...
    PVC        & 60/60 & 100.0\% & \\
    AFib       & 60/60 & 100.0\% & \\
    VT         & 58/60 & 96.7\% & 2 misclassified as PVC \\
    SVT        & 59/60 & 98.3\% & 1 misclassified as AFib \\
    \midrule
    \textbf{Total} & \textbf{296/300} & \textbf{98.67\%} & 0 quantization flips \\
    \bottomrule
  \end{tabular}
\end{table}

The primary confusion is VT misclassified as PVC, consistent with the
similarity of wide-QRS morphology on a single-beat basis.

\subsection{Non-Uniform Allocation Sweep}

Table~\ref{tab:alloc} shows that the uniform allocation is optimal at 600 SVs.
Non-uniform splits either match or degrade accuracy; concentrating more SVs in
one class introduces tail noise without improving any decision boundary.

\begin{table}[H]
  \centering
  \caption{600-SV non-uniform allocation sweep.}
  \label{tab:alloc}
  \small
  \begin{tabular}{lrrl}
    \toprule
    ...
    \midrule
    $[120,120,120,120,120]$ (uniform) & 98.67\% & \textbf{98.67\%} & 0 \\
    $[140,115,115,115,115]$ (N boost) & 98.67\% & 98.67\% & 0 \\
...
\noindent\textbf{Cycles per beat formula.}  Let $F=256$, $\SV=600$, $\Lat=3$:
\begin{align}
  T &= F(\Lat+1) + \SV\!\bigl[F(\Lat+1) + 30\bigr] + 500 \notag\\
    &= 256\cdot4 + 600\cdot(256\cdot4+30) + 500 \notag\\
    &= 1024 + 600\cdot1054 + 500 \notag\\
    &= \mathbf{633{,}924\text{ cycles/beat}}. \label{eq:cycles}
\end{align}
At $f_\text{clk}=40\MHz$: $T/f = 15.85\,\mathrm{ms/beat}$, well within the
$750\,\mathrm{ms}$ heartbeat window at 80\,bpm.
...
\noindent\textbf{Startup sequence.}  The host MCU performs 614 SPI write frames
before asserting \texttt{CONTROL[start]}:

\begin{equation}
  N_\text{frames} = \underbrace{600}_{\texttt{ALPHA\_WR}}
...
At 2\,MHz SCLK and 40-bit frames, startup overhead is $614 \times 20\,\mu\mathrm{s}
= 12.3\,\mathrm{ms}$.
...
Rows 0--599 hold the SV matrix; rows 600--1599 hold up to 1000 input beats.
\texttt{RAM\_LATENCY=3} inserts three wait-state cycles between
\texttt{ram\_ren\_out} assertion and valid \texttt{ram\_rdata\_in}, matching the
device's $10\,\mathrm{ns}$ access time at $25\,\mathrm{ns}$ clock period.
...
\begin{enumerate}[noitemsep]
  \item Writes 600 alpha coefficients via \texttt{ALPHA\_WR} (600 SPI frames).
...
Result: \textbf{296/300 correct (98.67\%)}, zero quantization flips vs.\ float.
...
During SVM inference all $\SV=600$ support vector comparisons are active.
The analytical estimate scales from the measured sky130A m5 result using
supply voltage:

\begin{equation}
  P_{\text{active}} \approx P_{\text{m5}}\cdot\left(\frac{V_{\text{IHP}}}{V_{\text{sky130}}}\right)^2
  = 55.25\mW \times \left(\frac{1.2}{1.8}\right)^2
  = 55.25 \times 0.444 \approx 24.5\mW.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrl}
    ...
    \midrule
    SVM core (600 SVs, LAT=3) & $\approx 24.5\mW$ & 2.11\% & $\approx 0.52\mW$ \\
    ...
    \midrule
    \textbf{Total} & --- & --- & $\approx\mathbf{1.12\mW}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{equation}
  t_{\text{battery}} = \frac{740\,\mathrm{mWh}}{1.12\,\mathrm{mW}} \approx 661\,\mathrm{h} \approx 27.5\,\mathrm{days}.
  \label{eq:battery}
\end{equation}
The 14-day target is met with $1.96\times$ margin.  Compared to the m5 sky130A
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Total SVs & 500 $[95,95,95,120,95]$ & \textbf{600} $[120,120,120,120,120]$ \\
    alpha\_addr width & 9-bit & \textbf{10-bit} \\
    Die & Caravel fixed $2920\times3520\um$ & \textbf{Standalone} $2400\times2400\um$ \\
    Accuracy (Q6.10) & 98.33\% (295/300) & \textbf{98.67\%} (296/300) \\
    Active power (est.) & 55.25\,mW & \textbf{$\approx$24.5\,mW} \\
    Avg power @ 80\,bpm & 0.869\,mW & \textbf{$\approx$0.52\,mW} \\
    Inference time (LAT=3) & 9.87\,ms/beat & 15.85\,ms/beat ($+$6\,ms for 100 SVs) \\
...
SPI without resetting the device.  The off-chip SRAM ($307.2\,\mathrm{KB}$ for
the SV matrix) and the on-chip alpha table ($1.2\,\mathrm{KB}$) are overwritten
in place while the FSM is idle (\texttt{STATUS[done]=1}):

\begin{enumerate}[noitemsep]
  \item Write SV matrix to SRAM via MCU direct SRAM bus ($600\times256\times16$-bit).
  \item Write 600 alpha coefficients via \texttt{ALPHA\_WR} ($[25:16]$=SV index, $[15:0]$=Q6.10 $\hat\alpha$).
