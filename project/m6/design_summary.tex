...
\texttt{LOAD\_INPUT}  & $F \cdot L = 768$   & Fetch input feature vector from SRAM \\
\texttt{ACCUMULATE}   & $F \cdot L + 2 = 770$ & Compute $D$ for one SV (with 2-cycle drain) \\
\texttt{KERNEL}       & $\approx 18$        & LUT lookup: $K = \exp(-\gamma D)$ \\
...
\begin{align}
  T &= F\Lat + \SV\!\bigl[F\Lat + 20\bigr] + \text{overhead} \notag\\
    &\approx 768 + 500 \cdot 788 + 1{,}769 \notag\\
    &= \mathbf{396{,}769\text{ cycles/beat}}. \label{eq:cycles}
\end{align}
The per-SV term $+20$ captures the 2-cycle pipeline drain and the 18-cycle
KERNEL Horner evaluation; the 1{,}769-cycle overhead represents FSM
state-transition latency.  This matches the m5 benchmark cycle-accurate table
(LOAD: 768 + COMPUTE\_DIST: 387{,}000 + KERNEL: 9{,}000 + WRITE: 1
$=$ 396{,}769 cycles).
At $f_\text{clk}=40\MHz$: $T/f \approx 9.9\,\mathrm{ms/beat}$,
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Active power (est.) & 66\,mW & \textbf{$\approx$29\,mW} \\
    Avg power @ 80\,bpm & 0.852\,mW & \textbf{$\approx$0.38\,mW} \\
    Inference time (LAT=3) & $\approx$9.9\,ms/beat & \textbf{$\approx$9.9\,ms/beat} \\
...
All place-and-route, timing signoff, and cosimulation jobs were run on the
\emph{Orca} high-performance computing cluster at Portland State University
(PSU Research Computing), using LibreLane 3.0.4 inside an Apptainer
container on the \texttt{long} SLURM partition.

\medskip
\noindent\textbf{Data.}
The cardiac arrhythmia classifier was trained and evaluated using the following
publicly available resources:

\begin{itemize}[noitemsep]
  \item \textbf{MIT-BIH Arrhythmia Database} (PhysioNet record nos.\ 100--234):
        G.\ B.\ Moody and R.\ G.\ Mark, ``The impact of the MIT-BIH Arrhythmia
        Database,'' \emph{IEEE Eng.\ Med.\ Biol.\ Mag.}, vol.\ 20, no.\ 3,
        pp.\ 45--50, 2001.
  \item \textbf{PhysioNet / PhysioBank / PhysioToolkit}:
        A.\ L.\ Goldberger \emph{et al.}, ``PhysioBank, PhysioToolkit, and
        PhysioNet,'' \emph{Circulation}, vol.\ 101, no.\ 23,
        pp.\ e215--e220, 2000.  \url{https://physionet.org}
  \item \textbf{IHP SG13G2 Open-Source PDK}:
        IHP Microelectronics (Leibniz-Institut für innovative Mikroelektronik),
        \emph{ihp-sg13g2 Open-Source PDK}, Frankfurt (Oder), Germany, 2024.
        \url{https://www.ihp-microelectronics.com} $\cdot$
        \url{https://github.com/IHP-GmbH/IHP-Open-PDK}
  \item \textbf{LibreLane EDA flow}:
        Efabless / The OpenROAD Project, \emph{LibreLane 3.0.4},
        \url{https://github.com/efabless/openlane2}, 2024.
\end{itemize}
