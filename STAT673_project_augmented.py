...
for i in range(0,n_trajectories):
    ...
    G=np.column_stack([F_theta[0:np.size(tt):ns],F_omega[0:np.size(tt):ns],np.ones([int(np.size(tt)/ns),1])])
...
def k(x,y,gamma=.1,p=20): #define kernel
...
d=3 #dimension of system variable,yt.y=yt.[theta,omega,dt/dt=1]   
...
l=100 #regularization hyperparmeter.
...
y_0=np.array([[theta_00*i,0,1] for i in range(n_trajectories)]) #Inital condition for each trajectory.
