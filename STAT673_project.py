...
omega_0=.1*2.5 #contant equal to sqrt(g/l) where l is pendulum length.
t=range(100) #time length of tranjectories
n_trajectories=15 #number of trajectories
n_samples=50 #number of points samples from each trajectory
...
Gmatrix=np.squeeze(np.stack(Gmatrix)) #Combine all trajectory data in to one matrix
...
d=2 #dimension of system variable,yt.y   
delta=(Gmatrix[:,-1]-Gmatrix[:,0]).reshape(d*n_trajectories,1)
        
Marray=np.kron(M,np.identity(d)) #create matrix of shape(n_trajectoriesxd,n_trajectoriesxd).
...
#inegrate system as specified using MOCK

y_0=np.array([[theta_00*i,0] for i in range(n_trajectories)]) #Inital condition for each trajectory.
...
for i in range(n_trajectories):
    ...
    yt=solve_ivp(fstar,t_span=(0,np.size(t)),t_eval=[i for i in range(100)],y0=y_0[i],args=(Gmatrix,alpha,np.size(t),n_trajectories),vectorized=True)
    plt.plot(yt.t,np.squeeze(yt.y[0]))
    plt.show()
