...
module tb_crossbar_sds;
    ...
    // Intermediate storage for superposition test
    reg signed [9:0] ia0, ia1, ia2, ia3;
    reg signed [9:0] ib0, ib1, ib2, ib3;

    initial begin
        fd = $fopen("crossbar_sds_sim.log", "w");
        if (fd == 0) begin $display("ERROR: could not open log"); $finish; end

        pass = 0; fail = 0;

        `LOG("=== crossbar_sds simulation log ===")
        `LOG("DUT: crossbar_sds  --  Split Differential Sensing, W.T @ v  (4x4, entries +/-1)")
        `LOG("W.T:  i0=[+1,+1,-1,-1]  i1=[-1,+1,+1,-1]  i2=[+1,-1,+1,-1]  i3=[-1,-1,-1,+1]")
        `LOG("det(W.T) = -8  =>  full rank  =>  trivial null space")
        `LOG("")

        // ── Section 1: Basis vectors ───────────────────────────────────────
        // Correctness for all inputs follows by linearity from these four.
        ...
        // Verify W.T@(a+b) = W.T@a + W.T@b for one non-trivial pair.
        ...
        // Worst-case inputs to verify 10-bit output does not wrap.
        // Note: signed 8-bit asymmetry (max=127, min=-128) means coupled
